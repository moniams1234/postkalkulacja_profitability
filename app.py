...
# Custom two-way sidebar toggle. The button remains visible in the main area,
# so the user can restore the sidebar after hiding it.
_sidebar_hidden = st.session_state.get("sidebar_hidden", False)
...
if _sidebar_hidden:
    ...
    .sidebar-toggle-box {
        ...
        border: 1px solid rgba(107,0,0,.18);
        border-radius: 12px;
        padding: 4px 6px;
...
# Button is intentionally outside st.sidebar, so it is still accessible when sidebar is hidden.
st.markdown('<div class="sidebar-toggle-box">', unsafe_allow_html=True)
if st.button("☰ Pokaż sidebar" if _sidebar_hidden else "◀ Ukryj sidebar", key="_toggle_sidebar_visibility"):
