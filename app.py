...
def clear_user_inputs() -> None:
    ...
    for key in list(st.session_state.keys()):
        if any(key == base or key.startswith(f"{base}_") for base in upload_bases):
    ...
    st.session_state["pp_digital"] = 10.0
    st.session_state["pp_offset"] = 40.0
...
def recalculate_profitability(show_success: bool = True) -> bool:
    ...
    with st.spinner("Przeliczanie profitability…"):
        result, warns = build_profitability(
            uploaded_file("uf_base"),
            uploaded_file("uf_czasy"),
            uploaded_file("uf_zlec"),
            uploaded_file("uf_fry"),
            uploaded_file("uf_inks"),
            uploaded_file("uf_farby"),
...
with st.sidebar:
    ...
    # ── Status panel ──────────────────────────────────────────────────────
    base_ok   = uploaded_file("uf_base")  is not None
...
if page == PAGES[0]:
    ...
    with col1:
        ...
        st.file_uploader("Baza / post_list ⭐", type=["xlsx","xls"], key=upload_key("uf_base"))
        st.markdown("#### 🏭 Produkcja & Faktury")
        st.file_uploader("Czasy dla aplikacji",  type=["xlsx","xls"], key=upload_key("uf_czasy"))
        st.file_uploader("Zlecenia + faktury",   type=["xlsx","xls"], key=upload_key("uf_zlec"))
        st.file_uploader("Faktury – linie",      type=["xlsx","xls"], key=upload_key("uf_fry"))
    with col2:
        st.markdown("#### 💰 Koszty")
        st.file_uploader("Kliki / Inks",               type=["xlsx","xls"], key=upload_key("uf_inks"))
        st.file_uploader("Farby podsumowanie (Offset)", type=["xlsx","xls"], key=upload_key("uf_farby"))
    ...
    for i, (label, key) in enumerate(FILES_STATUS):
        ok = (uploaded_file(key) is not None) if key.startswith("uf_") else (st.session_state.get(key) is not None)
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    if st.button("💾 Zapisz Prepress"):
        ...
        for _, row in edited_pp.iterrows():
            ...
            if kl:
                new_pp[kl] = {
                    "digital": sn(row.get("Digital", st.session_state.get("pp_digital", 10.0))),
                    "offset":  sn(row.get("Offset",  st.session_state.get("pp_offset", 40.0))),
                }
        st.session_state["prepress"] = new_pp
        st.session_state["settings_changed"] = True
        st.success("✅ Prepress zapisany!")

    recalc_box("Prepress lub stawki per klient zostały zmienione. Kliknij przelicz, aby wynik Profitability użył aktualnych ustawień. Pliki pozostają załadowane.")
