...
st.markdown("""
<style>
/* Sidebar visibility + readable nav labels */
[data-testid="stSidebar"] {
...
# Widoczna zawsze w głównej części aplikacji, nawet gdy sidebar Streamlit jest
# zwinięty lub niewidoczny po wdrożeniu w przeglądarce.
st.markdown("""
<style>
.top-nav-card {
    background: #FFFFFF;
    ...
    box-shadow: 0 2px 12px rgba(107,0,0,.06);
}
.top-nav-title {
    color: #6B0000;
    font-size: .78rem;
...
<div class="top-nav-card">
    <div class="top-nav-title">Nawigacja / zakładki</div>
</div>
""", unsafe_allow_html=True)

# One shared page state used by both top navigation and sidebar.
if "page_selected" not in st.session_state:
    st.session_state["page_selected"] = PAGES[0]

if st.session_state["page_selected"] not in PAGES:
    st.session_state["page_selected"] = PAGES[0]

_top_idx = PAGES.index(st.session_state["page_selected"])
page = st.selectbox(
    "Wybierz zakładkę",
    PAGES,
    index=_top_idx,
...
with st.sidebar:
    ...
    _side_idx = PAGES.index(st.session_state.get("page_selected", PAGES[0])) if st.session_state.get("page_selected", PAGES[0]) in PAGES else 0
    side_page = st.radio(
        "Nawigacja",
        PAGES,
        index=_side_idx,
    ...
    if side_page != st.session_state.get("page_selected"):
        st.session_state["page_selected"] = side_page
        page = side_page
